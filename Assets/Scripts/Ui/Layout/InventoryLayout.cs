...
namespace Ui.Layout
{
    public class InventoryLayout : LayoutGroup
    {
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            if (setParentWidth)
            {
                float width = cellSize.x * columns;
                SetLayoutInputForAxis(width, width, -1, 0);
            }
            if (setParentHeight)
            {
                float height = cellSize.y * rows;
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                Vector2 pos = new Vector2(i % columns, Mathf.Floor(i / (float) columns)) * cellSize;
